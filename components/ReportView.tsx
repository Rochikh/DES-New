  ...
  return (
    <div className="min-h-screen bg-white p-4 sm:p-12 print:p-0">
      <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-700 print:space-y-8">
        ...
        <section className="pt-12 border-t border-slate-100 no-print">
          <div className="flex items-center gap-3 mb-8">
            <Target className="text-slate-900" size={24} />
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Dialogue d'Origine</h3>
          </div>
          <div className="space-y-4 max-h-96 overflow-y-auto pr-4 scrollbar-hide opacity-60 hover:opacity-100 transition-opacity">
            {transcript.filter(m => !m.text.includes("Bonjour Argos")).map((m, i) => (
              <div key={i} className={`p-4 rounded-xl text-xs ${m.role === 'user' ? 'bg-slate-100 border-l-4 border-slate-900' : 'bg-indigo-50 border-l-4 border-indigo-600'}`}>
                <span className="font-black uppercase text-[9px] block mb-1">{m.role === 'user' ? 'Étudiant·e' : 'Argos'}</span>
                {m.text}
              </div>
            ))}
          </div>
